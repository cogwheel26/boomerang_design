...
| Cryptographic storage                           | Phone doxing data, SAR DB, Niso disk                    | Authenticated encryption, key rotation, encryption-at-rest, HSM usage for services.   | `doxing_key` currently uses `SHA256(password)`, making it brute-forceable if leaked.              | T-CRYPTO-03,T-DATA-01 / R-07,R-06  |
...
| DG-11 | Dynamic doxing data integrity is largely delegated to the phone and SAR operations | Rescue quality depends on phone-sourced data staying correct and current, but the current design gives little direct protection against phone compromise or feed stoppage. | High | Open |
...
| Gap ID | Design gap | Why it matters now | Priority | Status / Resolution |
| --- | --- | --- | --- | --- |
| DG-16 | Privacy leakage is inventoried but not yet turned into a minimization strategy | The current leakage analysis shows a large sensitive-data surface area, but the design does not yet consolidate which disclosures are acceptable, avoidable, or must be redesigned. | High | Open |
