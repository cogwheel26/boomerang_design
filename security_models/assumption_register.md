...
- `FM-28`: recurring duress cadence follows the design's modulo trigger, but the PRNG itself is still abstract.
...
| DG-04 | `AR-27`, `AR-28`, `AR-71` | Boomletwo activation and anti-clone semantics are unspecified | `SPEC.md` defines authenticated backup import excluding active `mystery`, but the safe way to deactivate Boomlet and activate Boomletwo without creating duplicate authority is not yet designed. |  | Critical |
